...
public:
 void DeleteAll() { 
  ALCDeviceContext *s,*n;
  for ( s=(ALCDeviceContext *)first; s; s=n ) { n=(ALCDeviceContext *) s->next; delete s; }
  first=null;  last=null;  count=0;
 }
  ~ALCDeviceContexts() {
  ALCDeviceContext *d,*n;
  for ( d=(ALCDeviceContext *) first; d; d=n ) {
   n=(ALCDeviceContext *) (d->next);
   delete d;
  } 
 }
};
...
{
public:
 bool inList(char *fn) { ALDeviceDescription *a; for (a=(ALDeviceDescription *) first; a; a=(ALDeviceDescription *) (a->next) ) if ( contains(a->name, fn) ) return true; return false; }
 ALDeviceDescription *find(char *fn) { ALDeviceDescription *a; for (a=(ALDeviceDescription *) first; a; a=(ALDeviceDescription *) (a->next) ) if ( contains(a->name, fn) ) return a; return null; }
 ~ALDeviceDescriptions() {
  ALDeviceDescription *d,*n;
  for ( d=(ALDeviceDescription *) first; d; d=n ) {
   n=(ALDeviceDescription *) (d->next);
   delete d;
  }
 }
};
...
// Timer timer { this, delay=(1.0f/refreshRate); started=false;
 virtual void Virtual()
 {
  ...
  for ( s=(ALSound *) (sounds->first); s; s=(ALSound *) (s->next) ) s->BetweenFrames();
  sounds->DeleteDone();
 }
// };
 ...
 ALSoundManager() {
  ...
  waveforms=new ALWaveForms;
  streams=new ALUREStreams;
  sounds=new ALSounds;
  deviceContexts=new ALCDeviceContexts;
  outputs=new ALDeviceDescriptions;
  inputs =new ALDeviceDescriptions;
 ...
 {
  ALSound *s;
  for (s=(ALSound *)(sounds->first); s; s=(ALSound *)(s->next) ) { s->Release(); }
  delete sounds;
 ...
 bool Init( ALCDeviceContext *alc ) {
  ...
  else { alc=new ALCDeviceContext; alc->PickDefault(); deviceContexts->Append(alc); }
...
//  int newid;
  ...
  alGetError();
//  if ( unusedBuffers <= 0 ) { OUTPUT( "ALSound:Cue() No more buffers available (%d used)\n", bufferClamp-unusedBuffers ); return null; }
  sound = new ALSound;
  sound->pitch=pitch;
  sound->gain=gain;
  sound->loop=(ALboolean) looped;
  sprintf_s( sound->name, "%s", filename);
  sound->waveform=waveforms->findOrLoad(path,filename);
...
//   OUTPUT ( "Playing sound '%s' buffer %d source %d\n", s.name, (int) s.buffer.id, (int) s.source.id );
 ...
 ALUREStream *AddStream( char *fn, ALSource *source, int loops, bool calcLength ) {
  ...
  streams->Append(stream);  //  incref stream; (eC)
  stream->manager=this;
  stream->loops=loops;
  if ( !stream->LoadIntoSource(fn,stream->source,calcLength) ) { OUTPUT("ALSoundManager:AddStream:stream.Create() had an error.\n" ); return stream; }
  return stream;
 }
 ...
 bool StopAll( void ) {
  ALSound *s;
  alGetError();
  for ( s=(ALSound *) (sounds->first); s; s=(ALSound *) (s->next)) if ( s->Playing() ) s->Stop();
 ...
 void PrintDevices() {
  ...
  OUTPUT( "Inputs: ");
  for ( ALDeviceDescription *a=(ALDeviceDescription *)(inputs->first);  a; a=(ALDeviceDescription *)(a->next) ) OUTPUT( "%s%s", a->name, a->next ? "s" : "");
  OUTPUT( "\nOutputs: ");
  for ( ALDeviceDescription *a=(ALDeviceDescription *)(outputs->first); a; a=(ALDeviceDescription *)(a->next) ) OUTPUT( "%s%s", a->name, a->next ? "s" : "");
  OUTPUT( "\n" );
 }
 ...
 void ShowDopplerEquation() { OUTPUT( "ALDopplerEquation: shift = DOPPLER_FACTOR * freq * (DOPPLER_VELOCITY - l.velocity) / (DOPPLER_VELOCITY + s.velocity) \n" ); }
 void SetDoppler( float Factor, float velocity ) {
  alDopplerFactor  ( dopplerFactor=Factor );     if(ALError("ALSoundManager:SetDoppler:alDopplerFactor"  )){}
  alDopplerVelocity( dopplerVelocity=velocity ); if(ALError("ALSoundManager:SetDoppler:alDopplerVelocity")){}
 }
};
...
//  return true;
 ...
 inline void Load( char *fn )       { manager.waveforms->findOrLoad(path,fn); }
 inline void findOrLoad( char *fn ) { manager.waveforms->findOrLoad(path,fn); }
...
public:
 char *sound;
 ALUREStream *stream;
 bool loop, fire_and_forget;
 int sampleLength;
 int filesize;
 float elapsed,delay,mark;
 bool expired,played;

 SoundEvent() {
  sound=null;
  stream=null;
  loop=fire_and_forget=false;
  sampleLength=0;
  filesize=0;
  elapsed=delay=mark=0.0f;
  expired=played=false;
 }
 void Init( ) { 
 }
 virtual void OnMark() { }
 ~SoundEvent () { 
  ...
  if ( stream ) delete stream; /*delete stream; */ }
};
...
class SoundSequencer : public LinkedList {
public:
 Audio *audio;
 SoundEvents events;
 SoundSequencer() { audio=null; }

 void Add( SoundEvent *s ) { events.Append(s); s->Init(); }
 void Update( float rate ) { 
  SoundEvent *e;
  for (e=(SoundEvent *)first; e; e=(SoundEvent *)e->next) {
   if ( !e->expired && ( e->elapsed+=rate ) >= e->delay+e->mark ) { e->expired=true; e->OnMark(); } else { 
    if ( e->elapsed >= e->delay ) { 
     if ( e->stream && !e->stream->Playing() ) { e->stream->Play(); } 
     if ( e->sound && !e->played ) { audio->manager.CueAndPlay( e->sound, 1.0f, 1.0f, false, true ); e->played=true;}
    }
   }
  }
  return; 
 ...
 void Stop()   { 
  SoundEvent *e;
  for (e=(SoundEvent *)first; e; e=(SoundEvent *)e->next) if ( e->stream && e->stream->source && e->stream->source->valid() && e->stream->source->state == playing ) e->stream->Stop(); 
 }
 void Pause()  { 
  SoundEvent *e;
  for (e=(SoundEvent *)first; e; e=(SoundEvent *)e->next) if ( e->stream && e->stream->source && e->stream->source->valid() && e->stream->source->state == playing ) e->stream->Pause();
 }
 void Resume() {
  SoundEvent *e;
  for (e=(SoundEvent *)first; e; e=(SoundEvent *)e->next) if ( e->stream && e->stream->source && e->stream->source->valid() && e->stream->source->state == paused  ) e->stream->Resume();
 }
 void Reset()  {
  SoundEvent *e;
  for (e=(SoundEvent *)first; e; e=(SoundEvent *)e->next) { e->elapsed=0.0f; e->played=false; if ( e->stream && e->stream->source && e->stream->source->state != stopped ) e->stream->Rewind(); } 
 }
 ~SoundSequencer() { 
  Stop(); 
  events.Free(); 
